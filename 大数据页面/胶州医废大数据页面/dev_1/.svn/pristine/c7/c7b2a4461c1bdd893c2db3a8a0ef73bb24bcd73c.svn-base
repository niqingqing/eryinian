...
var stepBar = {
    ...
    reset: function () {
        ...
        if (this.curStep >= this.step - 1) {
            var _this = this;
            setTimeout(function(){
                _this.bar.find(".ui-stepProcess").width("0%");
                _this.curStep = -1;
                // this.bar.find(".ui-stepInfo .ui-stepSequence").removeClass(hindClass).addClass(preClass);
                _this.bar.find(".ui-stepInfo .ui-stepSequence").removeClass("judge-stepSequence-pre").addClass("judge-stepSequence-hind");
                setTimeout(initStepBar, 1000);
                // initStepBar();
            }, 3000);
