...
var commonVue={
    ...
    tagTableHeaderClick: function tagTableHeaderClick(column, event) {
        ...
        if (column.label.trim() == lang['view'] && event.target.nodeName != "INPUT") {
    ...
    fenceTableHeaderClick: function (column, event) {
        if (column.label.trim() == lang['view'] && event.target.nodeName != "INPUT") {
            this.handleCheckAllChange('fence');
        }
    },
    ...
    * */
    ...
    onlyShowActiveTag:function (flag) {
        if(flag){
            ...
            ME.vm.handleCheckAllChange('tag',true);
        }
    },
    ...
    handleCheckChange: function handleCheckChange(type, item, def) {
        ...
        if(type == 'fence'){
            if(item.ftypeId>3 && item.ftypeId<7){
                return;
            }
            if(item.isShow){
                FenceTool.showFence(item.id, {force: true}, ME.vm.mapId);
            }else{
                FenceTool.hideFence(item.id,{force:true});
            }
        }
        ...
        for (var key of Object.keys(setData)) {
            if (_typeof(setData[key]) === 'object') {
                ...
                if (keyList) {
                    ...
                    if(prevKey === '_tagCat' || prevKey === 'tagCat'||prevKey ==='_ftypeId'){
                        continue;
                    }
                    ...
                    if (int2str && !(getData[key]===undefined||getData[key]===null)) {
    ...
    confirmBox:function (msg,param) {
        ME.vm.$confirm(msg, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        }).then(() => {
            // console.log(param);
            var url = window.location.href.split("?")[0]+'?map='+param.mapId + "&searchName="+param.searchName;
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                if (searchData) {
                    ...
                    // ubiMap.pan.applyZoom(oldCurrentZoom-newCurrentZoom,{ x: document.body.clientWidth / 2, y: document.body.clientHeight / 2 });
                ...
                DataManager.searchMarkers(searchName, function(datas){
                    ...
                    if(markers.length == 1){
                        ...
                        if(!searchData.mapId){
                            ME.vm.showTip("标签\""+ searchName +"\"未上过线！");
                            return;
                        }
                        if (searchData.mapId != ME.currentMapId) {
                            ...
                            if(searchData.markerType == 'tag'&&searchData.online == '0'){
                                msg +=",目前已经离线";

                            }
                            var param={mapId:searchData.mapId,searchName:searchName};
                            ME.vm.confirmBox(msg+'。确认跳转地图查看？',param);
                            return;
                        }
                        ...
                        if(searchData.markerType == 'tag'&&searchData.online == '0'){
                            var map = ME.vm.getMapById(searchData.mapId);
                            var lastPosTime = (new Date(parseInt(searchData.time))).Format("yyyy-MM-dd hh:mm:ss");
                            var msg = lang['searchMarkersNotePre2']  + map.cname + lang['searchMarkersNoteAfter2']+lastPosTime+"）";
                            ME.vm.showTip(msg+"。目前标签\""+ searchName +"\"离线！");
                        }
                        return;
                    }
    ...
    //确定提交方法
    submit: function submit(id, key, data, callback,editKey) {
        // debugger;
        var vm = this;
        ...
        if (!id) {
            ...
            service(key).add(postData).then(util.jsonFunc).then(function (json) {
                if (json.isOk) {
                    ...
                    if (callback) callback(key, 'add', postData, json,editKey);
            ...
            service(key).update(postData).then(util.jsonFunc).then(function (json) {
                if (json.isOk) {
                    if (callback) callback(key, 'update', postData, json,editKey);
                    if (json.data) {
                        vm[key].data.forEach(item => {
                    ...
                    if(key == 'edit'){
                        FenceTool.showFence(postData.id,{force:true},ME.vm.mapId);
                    }
                    ME.vm.showTip(json.msg);
                }
            });
    ...
    fenceCopy: function (row,event) {
        if(this.editLastFenceId){
            try {
                FenceTool.showFence(this.editLastFenceId, {force: true}, ME.vm.mapId);
            }catch (e){
                console.log(e);
            }
        }
        SysAlert.setWaitMessage(lang['escCancel'], "info", !0, "long");
        this.clearTmpFence();
        var fenceType = 'edit';
        ...
        util.emptyObj(this['fence'].edit);
        this.fence[fenceType].visible = true;
        this.fence[fenceType].trif = 'io';
        this.fence[fenceType].isRecord = '1';
        this.fence[fenceType].isEmail = '0';
        this.fence[fenceType].isSMS = '0';
        ubiMap.removeUnsavedPolygons();
        this.tempCopyFence(row);
        var moveRect=null;

        var points = this.getPolyPoints(this.polygonPoints,'unconvert');
        FenceTool.createNewFence("new", points, [], points.length-1, !1, ME.vm.mapId);
    },
    tempCopyFence:function (row) {
        var polygonPoints =[];
        for(var i =0;i<row.polygonPoints.length;i++){
            var pt = {
                x:row.polygonPoints[i].x,
                y:row.polygonPoints[i].y
            };
            polygonPoints.push(pt);
        }
        this.polygonPoints = polygonPoints;
        this.getPolyPoints(this.polygonPoints,'unconvert');
        this.fenceDrawing = paper.Fence(polygonPoints,ME.vm.fence.color,ME.vm.fence.opacity);
        this.bindFencePonits();
    },
    getPolyPoints:function (points,func) {
        points=points||this.polygonPoints;
        func=func||'unconvert';
        var newPoints=[];
        if(func == 'convert'){
            for(var i=0;i<points.length;i++){
                var point = this.newPoint(points[i],func)
                var newPoint ={
                    relX:points[i].x,
                    relY:points[i].y,
                    x:point.x,
    ...
    newPoint:function (position,func) {
        if(func =="convert") {
            ...
            return newPos;
        }else if(func =="unconvert"){
            if (!ME.vm.scanActive) {
                position.y -= ME.vm.yCenter;
            }
            var newPos = ubiMap.unconvert(position, ME.vm.curRealLength);

            return newPos;
        }
    },
    clearTmpFence:function (keepFence) {
        this.polygonPoints=[]; //围栏坐标，用于修改时记录。
        if(!keepFence) {
            if (this.fenceDrawing && this.fenceDrawing.entireSet && this.fenceDrawing.entireSet.remove) {
                this.fenceDrawing.entireSet.remove();
            }
        }
        this.fenceDrawing=null;//画的临时围栏。
        this.fenceMovePoints.forEach(function (val) {
            val.remove();
        });
        this.fenceMovePoints=[];//围栏要移动的顶点
        this.clickPolygonPtNum=undefined;//要修改的围栏坐标的位置。
        this.clickPolygonFlag=false;//是否正在修改一个围栏的坐标。
        ubiMap.setplacingStatus("-1");
    },
    bindFencePonits:function () {
        var that =this;
        var movePoints =[];
        var polygonPoints = that.polygonPoints;
        for(let i=0;i<polygonPoints.length-1;i++){
            var c = that.bindOnePonit(polygonPoints[i],i);
            movePoints.push(c);
        }
        var specialPonit = {x:(polygonPoints[0].x+polygonPoints[1].x)/2,y:(polygonPoints[0].y+polygonPoints[1].y)/2}
        var moveSpecialPoint = that.bindOneSpecialPonit(specialPonit,polygonPoints.length);
        movePoints.push(moveSpecialPoint);
        that.fenceMovePoints=movePoints;

    },
    bindOneSpecialPonit:function (pos,i) {
        var that =this;
        var c = paper.circle(pos.x, pos.y, 10);
        c.attr("fill", "green");
        c.node.onclick=function (e) {
            that.preventDefaut(e);
            if(that.clickPolygonFlag){
                return;
            }
            that.clickPolygonFlag = true;
            that.clickPolygonPtNum = i;
            ubiMap.setplacingStatus("updateAllPolygonPoint", null, null, null, 'fence');
        };
        return c;
    },
    bindOnePonit:function (pos,i) {
        var that =this;
        var c = paper.circle(pos.x, pos.y, 10);
        c.attr("fill", "red");
        c.node.onclick=function (e) {
            ...
            that.clickPolygonFlag = true;
            that.clickPolygonPtNum = i;
            ubiMap.setplacingStatus("updatePolygonPoint", null, null, null, 'fence');
        };
        return c;
    },
    delBindPoint:function (i) {
        this.fenceMovePoints[i].remove();
    },
    preventDefaut:function (e) {
        // e.stopPropagation(); //ie不兼容
        //以下是兼容写法
        e=window.event||e;
    ...
    fenceEdit: function (row,event) {
        ...
        FenceTool.hideFence(row.id,{force:true});
        this.editLastFenceId =row.id;
        this.tempCopyFence(row);
        ...
        if(row.validity){
            ...
            this.setValidity(validity,this.fence.edit.baseValidity,this.fence.edit.fenceValidities);
        }
        // 已设置的时间的回显
        // console.log("this.fence.edit===", this.fence.edit);
        this.baseFenceValidities = this.fence.edit.fenceValidities
    ...
    clearSiren: function clearSiren() {
        if($("#normal_jqxNotification")[0]) {
            $("#normal_jqxNotification").jqxNotification("closeAll");
        }
        if($("#fenceAlert_jqxNotification")[0]) {
            $("#fenceAlert_jqxNotification").jqxNotification("closeAll");
        }
        if($("#fenceTimeoutAlert_jqxNotification")[0]) {
            $("#fenceTimeoutAlert_jqxNotification").jqxNotification("closeAll");
        }
        if($("#sos_jqxNotification")[0]) {
            $("#sos_jqxNotification").jqxNotification("closeAll");
        }
        if($("#fall_jqxNotification")[0]) {
            $("#fall_jqxNotification").jqxNotification("closeAll");
        }
        if($("#aggregate_jqxNotification")[0]) {
            $("#aggregate_jqxNotification").jqxNotification("closeAll");
        }
        if($("#stillness_jqxNotification")[0]) {
            $("#stillness_jqxNotification").jqxNotification("closeAll");
        }
        if($("#heartAlert_jqxNotification")[0]) {
            $("#heartAlert_jqxNotification").jqxNotification("closeAll");
        }
        if($("#forceRemove_jqxNotification")[0]) {
            $("#forceRemove_jqxNotification").jqxNotification("closeAll");
        }
        if($("#distanceAlert_jqxNotification")[0]) {
            $("#distanceAlert_jqxNotification").jqxNotification("closeAll");
        }
        if($("#moreMonitor_jqxNotification")[0]) {
            $("#moreMonitor_jqxNotification").jqxNotification("closeAll");
        }
        ME.sos = {};
        ME.forceRemove = {};
        ME.heartAlert = {};
    ...
    addFenceValiItem:function(){
        ...
        this.baseFenceValidities.push(item);

    },
    delFenceValiItem:function(onlyKey){
        if(!onlyKey)return;
        var firstIndex = this.baseFenceValidities.findIndex(function(item){
            return item.onlyKey == onlyKey;
        });
        if(firstIndex<0) return;
        this.baseFenceValidities.splice(firstIndex,1);
    ...
    // 提交数据前获取围栏有效期
    getFenceValidities: function(isPrivate){
        // 怕憝以前的功能有影响，暂时添加参数isPrivate表示单独的处理
        var fenceValidities=[];
        if (isPrivate && this.baseFenceValidities.length) {
            this.baseFenceValidities.forEach(function (item){
                if (!item.validity) {
                    return;
                }
                fenceValidities.push(item.validity[0].Format('hh:mm:ss')+'_'+item.validity[1].Format('hh:mm:ss'));
            })
    ...
    setValidity:function(validity,baseValidity,otherValidities){
        ...
        for(var i=0;i<validity.length;i++){
            ...
            var item={onlyKey:onlyKey,validity:[
                new Date(1971,0,0,startArr[0],startArr[1],startArr[2]),
                new Date(1971,0,0,endArr[0],endArr[1],endArr[2])]};
    ...
    // 最远距离
    farthestDistance: function(pos){
        if(!this.switchData.farthestDistance||!pos)return;
        if(!this.statisticsPoint||Object.keys(this.statisticsPoint).length<1) {
            this.statisticsPoint=pos;
        }else{
            // 计算框选区域到基站的最大值
            this.farthestDistanceData = this.calFarthestDistance(this.statisticsPoint,pos);
            this.farthestDistanceShow=true;
            this.statisticsPoint={};
        }
    },
    // 计算框选区域到基站的最远距离
    calFarthestDistance: function (statisticsPoint, pos) {
        this.farthestDistanceData = 0; // 初始化
        // 地图中基站的坐标信息
        let anchors = ME.vm.anchor && ME.vm.anchor.data || [];
        // 按照国定规格对框选的区域进行划分，划分成指定长宽的小格，实际距离
        let sx, sy, ex, ey;
        [sx, ex] = statisticsPoint.relX > pos.relX ? [pos.relX, statisticsPoint.relX] : [statisticsPoint.relX, pos.relX];
        [sy, ey] = statisticsPoint.relY > pos.relY ? [pos.relY, statisticsPoint.relY] : [statisticsPoint.relY, pos.relY];
        // 以10厘米作为默认，统计点的坐标
        let points = [];
        const step = 10;
        for (let i = 0; i < Math.ceil((ex - sx) / step); i++) {
            for (let j = 0; j < Math.ceil((ey - sy) / step); j++) {
                points.push({x: sx + (i - 1) * step, y: sy + (j + 1) * step });
            }
        }
        console.log(points, statisticsPoint, pos, anchors);
        // 最近基站的距离
        let minListDistance = null;
        for (let i = 0; i < points.length; i++) {
            ...
            for (let j = 0; j < anchors.length; j++) {
                ...
                let dis = Math.sqrt((an.x - po.x) * (an.x - po.x) + (an.y - po.y) * (an.y - po.y) + an.z * an.z);
                if (minDistance == null || dis < minDistance) {
                    minDistance = dis;
                }
            }
            if (minListDistance == null || minListDistance < minDistance) {
                minListDistance = minDistance;
            }
        }
        return minListDistance;
    },

    statisticsShow:function(e){
        if((!ME.vm.switchData.realTimeStatistics && !ME.vm.switchData.farthestDistance) ||!ME.vm.statisticsPoint||Object.keys(ME.vm.statisticsPoint).length<1) return;
    ...
    cancelFarthestDistance: function () {
        this.farthestDistanceShow=false;
        this.statisticsList=[];
        if(paper.statistics){
            paper.statistics.remove();
        }
