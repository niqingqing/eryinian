...
function cssHeaderInit() {
    ...
    //后期二次开发,添加鼠标移动显示所有在线的标签类型
    $('.total_online_tag').mouseover(function () {
        let html = '',
            icon = 'location_blue.png';
        var typeIcons = {};
        for (var i = 0; i < ME.vm.tag.onlineData.length; i++) {
            var tag = ME.vm.tag.onlineData[i];
        ...
        $('.total_online_tag_table').html(html);
        $('.total_online_tag_table').css('display', 'block');
    });
    $('.total_online_tag').mouseout(function () {
        $('.total_online_tag_table').css('display', 'none');
    });
