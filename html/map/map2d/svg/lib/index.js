...
function dataInit(datas, key, vm) {
    ...
    if (_.isArray(datas)) {
        if(key == "tag"){
            //后期二次开发, 添加的变量赋值 所有在线标签
            let onlineTag = [];//在线标签
            for (let i = 0; i < datas.length; i++) {
                if(datas[i].status != "offline"){
                    onlineTag.push(datas[i]);
                }
            }
            vm[key].onlineData = onlineTag;
        }
        vm[key].data = datas;
    } else {
        vm[key] = datas;
...
function  websocketOnData(data, realLength, scan, yCenter){
    ...
    if (!tag) {
        ...
        if(tag.tagDisappear || ME.vm.cfg_cancleTagAnimateMove_spanTime) {
            var spanTime = ME.vm.cfg_cancleTagAnimateMove_spanTime ? ME.vm.cfg_cancleTagAnimateMove_spanTime :tag.tagDisappear;
            if (data.time - tag.time > spanTime) {
                var tagMarker = ubiMap.getMarker('tag_' + code);
                if (tagMarker) {
                    tagMarker.attr({transform: "t" + data.screenX + ',' + data.screenY});
                }
            }
        }
        tag.x = data.x;
    ...
    if (!ubiMap.lockTag) {
        ...
        marker.node.x=data.x;
        marker.node.y=data.y;
        marker.node.screenX=data.screenX;
        marker.node.screenY=data.screenY;
        if(isIn){
            if(ME.vm.switchData.isShowActiveTag){
                if(!tag.isShow) marker.attr({ transform: "t" + data.screenX + ',' + data.screenY });
                tag.isShow = true;
                marker.node.isShow = true;
        ...
        var clrLog = !tag.lastPostion || Math.abs(tag.x-tag.lastPostion.x)>300 || Math.abs(tag.y-tag.lastPostion.y)>300;
            if(marker.node.isShow){ubiMap.move('tag_' + code, data.screenX, data.screenY, true, marker, clrLog)};
