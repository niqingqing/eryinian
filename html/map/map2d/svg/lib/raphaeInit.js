...
//            根据多个点绘制直线,如果成员为三人则绘制三角形，否则绘制被监人到其余成员连线
        ...
        this.init = function () {
            ...
            paper.canvas.onmousemove = function (e) {
                ...
                if (func == 'polygon') {
                    ...
                    if (distance.point1 && !distance.point2) {
                        if (void 0 === a.myDistanceLine) {
                            a.myDistanceLine = a.drawLineWOcircle(distance.point1.x, distance.point1.y, ME.vm.spaceBarClickTimes == 2 ? (distance.point1.pageX * zoom + svgOrigin.x).toFixed(1) : ex, ME.vm.spaceBarClickTimes == 1 ? (distance.point1.pageY * zoom + svgOrigin.y).toFixed(1) : ey);
                        } else {
                            a.myDistanceLine.refresh(ME.vm.spaceBarClickTimes == 2 ? (distance.point1.pageX * zoom + svgOrigin.x).toFixed(1) : ex, ME.vm.spaceBarClickTimes == 1 ? (distance.point1.pageY * zoom + svgOrigin.y).toFixed(1) : ey);
                        }
                        //动态显示距离
                        var start = { x: (distance.point1.relX/100).toFixed(2), y: (distance.point1.relY/100).toFixed(2) },
                            end = { x: ME.vm.spaceBarClickTimes == 2 ? start.x : (relX/100).toFixed(2), y: ME.vm.spaceBarClickTimes == 1 ? start.y : (relY/100).toFixed(2) };
                        var dis = util.getDistance(start, end).toFixed(2);
                ...
                }else if(func == 'updatePolygonPoint'){
                    if(!ME.vm.clickPolygonFlag) return;
                    var d = self.pan.getCurrentPosition();
                    var ex = e.clientX * (1 - self.pan.getCurrentZoom() / 10) + d.x;
                    var ey = e.clientY * (1 - self.pan.getCurrentZoom() / 10) + d.y;
                    var screenPos ={x:ex,y:ey};
                    var i = ME.vm.clickPolygonPtNum;
                    ME.vm.polygonPoints[i] = screenPos;
                    if(i ==0){
                        ME.vm.polygonPoints[ME.vm.polygonPoints.length-1] = screenPos;
                    }
                    ME.vm.fenceDrawing.fillareaObj.remove();
                    for(var j=0;j<ME.vm.polygonPoints.length;j++){
                        ME.vm.delBindPoint(j);
                    }
                    ME.vm.fenceDrawing = paper.Fence(ME.vm.polygonPoints,ME.vm.fence.color,ME.vm.fence.opacity);
                }else if(func == 'updateAllPolygonPoint'){
                    console.log(5)
                    ...
                    var ey = e.clientY * (1 - self.pan.getCurrentZoom() / 10) + d.y;
                    var screenPos ={x:ex,y:ey};
                    var specialPoint = {x:(ME.vm.polygonPoints[0].x+ME.vm.polygonPoints[1].x)/2,y:(ME.vm.polygonPoints[0].y+ME.vm.polygonPoints[1].y)/2};
                    var dx = screenPos.x-specialPoint.x;
                    var dy = screenPos.y-specialPoint.y;
                    for(var j=0;j<ME.vm.polygonPoints.length;j++){
                        ME.vm.polygonPoints[j].x +=dx;
                        ME.vm.polygonPoints[j].y +=dy;
                        ME.vm.delBindPoint(j);
                    }
                    ME.vm.fenceDrawing.fillareaObj.remove();
                    ME.vm.fenceDrawing = paper.Fence(ME.vm.polygonPoints,ME.vm.fence.color,ME.vm.fence.opacity);
            ...
            paper.canvas.ondblclick=function(e){
                ...
                if (ME.vm.switchData.realTimeStatistics) {
                    ME.vm.realTimeStatistics(pos);
                    return;
                }

                if (ME.vm.switchData.farthestDistance) {
                    ME.vm.farthestDistance(pos);
                    return;
                }
            },
            ...
            paper.canvas.onclick = function (b) {
                ...
                // 设置坐标原点
                    ...
                    var e=b;
                    ubiMap.setplacingStatus("-1", null, null, null, 'fence');

                    var mypoints = ME.vm.getPolyPoints();
                    FenceTool.createNewFence("new", mypoints, [], mypoints.length-1, !1, ME.vm.mapId);
                    ME.vm.bindFencePonits();
                    ME.vm.clickPolygonFlag =false;
                    ME.vm.clickPolygonPtNum =undefined;
        ...
        //将屏幕坐标转化为真实坐标
        this.unconvert = function (param, realLength) {
            if(!realLength) realLength = ME.vm.curRealLength;
            param.x = param.x ? param.x : 0;
            param.y = param.y ? param.y : 0;

            var screenX = parseFloat(param.x),
                screenY = parseFloat(param.y);

            var screenLength = this.screenLength;

            param.relPosX = parseFloat(screenX * realLength / screenLength);
            param.relPosY = parseFloat(screenY * realLength / screenLength);


            return { x: param.relPosX, y: param.relPosY };
        };
            //设置坐标
        ...
        this.move = function (id, x, y, p, marker, clrLog) {
            if (!marker) {
