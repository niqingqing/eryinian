...
var MarkerTool = {
    ...
    drawMarker: function drawMarker(allMarker, node, paper) {
        ...
        //dblclick事件
        MarkerTool.markerDblclick(marker);
        //hover事件
    ...
    markerClick: function markerClick(marker) {
        ...
        marker.click(function (event) {
            if(ubiMap.lockTag || ME.followMap || ME.dblclickAnchor){
                return false;
            }
            MarkerTool.marekerEditPanel(node);
        });
    },
    markerDblclick:function marckerDblclick(marker) {
        var node = marker.node;
    ...
    marekerEditPanel: function (node) {
        ...
        if(node.markerType == 'camera'){
            ...
            //控制编辑框显示
            var params={};
            let keyArray = ["ip", "id", "addUser", "addTime", "points", "mapId", "x", "y", "z", "port", "interfaceAgreement", "networkAgreement",
                "initAngleX", "rotateMax", "interfacePort", "username", "productModel", "seqNum", "manu", "markerType", "markerId"
            ]
            keyArray.forEach(v => {
                if (["x", "y", "z"].includes(v)){
                    params[v] = node[v] *100;
		...
		// params[v] = node[v];
             ...
             */
            // 摄像头安装方向 默认向上 key:"0"
            params.direction = node.direction || "0";
            node=params;
            setTimeout(function(){
    ...
    createMarkerInfoTable: function createMarkerInfoTable(marker, top, left) {
        ...
        var a = marker.node;
        var originRelPoint={x:ME.vm.currentMap.origin_x,y:ME.vm.currentMap.origin_y, z: ME.vm.currentMap.origin_z};
        ...
        c += "<tr><td>"+lang['locationM']+"</td><td>X:" + (a.x / 100).toFixed(2) + "，Y:" + (a.y / 100).toFixed(2) + "，Z:" + (a.z / 100).toFixed(2) + "</td></tr>";
        c += "<tr><td>"+lang['locationPx']+"</td><td>X:" + Math.round(a.screenX) + "，Y:" + Math.round(a.screenY) + "</td></tr>";
