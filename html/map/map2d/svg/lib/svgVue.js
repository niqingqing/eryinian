...
var subVue = {
    ...
    historyAnimate: function historyAnimate() {
        ...
        this.history.addCurrentTime = new Date(this.history.slider).Format('yyyy-MM-dd hh:mm:ss');//后期二次开发,添加变量,历史轨迹时间
        //时间播放 以秒为单位
    ...
    setAreaByType: function (e, key, keyType) {
        ...
        switch (key) {
            case "fence":
                ...
                this.fence.edit.fenceValidities=[];
    ...
    tagMessage:function () {
        this.tagcodes=[];
        this.tagcodes.push(this.tag.floatEdit.data.code);
    ...
    //anchor
    anchorSave: function anchorSave(event,type) {
        var _this2 = this;
    ...
    fenceSave: function(event,editKey) {
        ...
        if(editKey=='pollingEdit'){
            ...
            data.validity=ME.vm.getFenceValidities(true);
        }
        // 如果有效期存在，需要判重
        if (data.validity && data.validity.length) {
            let validityTime = data.validity.split(',');
            if (Object.keys(validityTime.reduce((da, item) => (da[item] = true) && da, {})).length !== validityTime.length) {
                // 存在相同时间的有效期
                this[key][editKey].visible = true;
                this.showTip("有效期存在重复，请修改后重新保存");
                return;
            }
        }
        ...
        if(data.id){
            ...
            var pointsArr = data.polygonPoints;
            var points='';
            for(var n=0;n<pointsArr.length;n++){
                if(n==pointsArr.length-1){
                    points +=Math.round(pointsArr[n].relPosX);
                    points += ' ';
                    points += Math.round(pointsArr[n].relPosY);
                }else{
                    points +=Math.round(pointsArr[n].relPosX);
                    points += ' ';
                    points += Math.round(pointsArr[n].relPosY);
        ...
        this.submit(data.id, key, data, this.fenceSubmitAfter,editKey);
    },
    //围栏绘制之后操作、维度区域
    fenceSubmitAfter: function fenceSubmitAfter(key, type, data, res,editKey) {
        ...
        if(key=='fence'){
            ...
            this[key]['edit'].visible = false;
            this[key].searchTrifTag = '';
            this[key].edit.trif_tags = [];
            this[key]['edit'].cname = '';
            this[key]['edit'].tagCat = '';
        }
        if (type == 'add') {
            data.id = res.id;
            data.savedInDb = true;
            ...
            for (var i = 0; i < ME.vm[key].data.length; i++) {
                ...
                if (data.id == item.id) {
                    ...
                    FenceTool.showFence(data.id,{force:true},ME.vm.mapId);
                    return
                }
            }
    ...
    fenceDelete: function fenceDelete(event, fenceId, type) {
        ...
        this.$confirm(lang['confrimDelete'] + msg + lang['fenceDeleteNoteAfter'], lang['prompt'], {}).then(function () {
            ...
            var index = _.findIndex(vm[key].data, ['id', id]);
            if(_this3.editLastFenceId == id){
                _this3.clearTmpFence();
            }
        ...
        if (this.power) {
            ...
            switch (key) {
                        ...
                        webSocketInit('2D', this.curRealLength,
                            {
                                ...
                                fenceTimeoutAlert:SysAlert.fenceTimeoutAlert,
                ...
                case 'realTimeStatistics':
                    if (status && ME.vm.switchData.farthestDistance) {
                        ME.vm.switchData.farthestDistance = false;
                    }
                    if(!status){
                ...
                case 'farthestDistance':
                    if (status && ME.vm.switchData.realTimeStatistics) {
                        ME.vm.switchData.realTimeStatistics = false;
                    }
                    if(!status){
                        this.cancelFarthestDistance();
                    }
            }
        }
    },
