...
var TABLE_COLUMNS = [
    ...
    {field: 'lde_cfg1_ntm_1', title: 'NTM1', width: '6%', sortable: true, searchable: true},
    {field: 'lde_cfg1_ntm_2', title: 'NTM2', width: '6%', sortable: true, searchable: true},
    {field: 'lde_repc', title: 'repc', width: '6%', sortable: true, searchable: true},
...
var VUE_DATA = {
    ...
    generateLonLatBtnDisable:false,
    ...
    basicLabels: ['projectId','productId','mapId','algorithm','min_num','anchor_signal','x','y','z','direction','lon','lat','netType','rxDelay@txDelay','alias','color'],
    basicSelected:[],//选择的属性
...
var VUE_METHODS = {
    generateLonLat:function () {
        ME.vm['generateLonLatBtnDisable'] = true;
        ME.vm.$http.post('project/anchor/generateLonLat', {}).then(function (res) {
            ME.vm['generateLonLatBtnDisable'] = false;
            var result = res.body;
            if (result.isOk) {
                this.$alert(lang['success'], lang['prompt']);
                this.refresh();
            } else {
                this.$alert(lang['fail'], lang['prompt'], {
    ...
    addAnchor:function(){
        ...
        this.form.lde_cfg1_ntm_1 = 'FF';
        this.form.lde_cfg1_ntm_2 = 'FFFF';
        this.form.lde_repc='FFFF';
        ME.addAnchors = [];
    },
    //添加提交方法 
    addDo: function () {
    ...
    setTransmit:function(){
        var params = this.getOnlineAnchorCodes();
        if(!params) return false;
        if(this.transmitSelected.indexOf('transmit')==-1&&this.transmitSelected.indexOf('NTM')==-1){
            ME.vm.$alert(lang['mustCheckParm'], lang['prompt'], {
                confirmButtonText: lang['confirm']
            });
            return false;
        ...
        if(this.transmitSelected.indexOf('transmit')!=-1){
            params.settings.channel=this.form.channel;
            params.settings.frameType=this.form.frameType;
            params.settings.dataRate=this.form.dataRate;
            params.settings.crc=this.form.crc;
            params.settings.pacSize=this.form.pacSize;
            params.settings.pulseFrequency=this.form.pulseFrequency;
            params.settings.preambleLength=this.form.preambleLength;
            params.settings.preambleCode=this.form.preambleCode;
            params.settings.smartPower=this.form.smartPower;
            params.settings.frameCheck=this.form.frameCheck;
        }
        if(this.transmitSelected.indexOf('NTM')!=-1){
            params.settings.lde_cfg1_ntm_1=this.form.lde_cfg1_ntm_1;
            params.settings.lde_cfg1_ntm_2=this.form.lde_cfg1_ntm_2;
            params.settings.lde_repc=this.form.lde_repc;
        }
    ...
    selectAnchors:function(codes){
        $('#anchorTable').bootstrapTable('checkBy', {field: 'code', values: codes});
    },
    filterAnchors:function (codes) {
        $('#anchorTable').bootstrapTable('filterBy', {'code': codes});
    },
...
function getMasterLists(anchors,tags) {
    ME.vm.masterLists = [];
    var seqNum={};
        ...
        if (a.isMaster > 0) {
            ...
            if (a.anchorSlotSeqNum || a.anchorSlotSeqNum === 0 || a.anchorSlotSeqNum === '0') {
                if (!seqNum[a.anchorSlotSeqNum]) {
                    ...
                    ME.vm.repeatSlotSeqNums.push(a.anchorSlotSeqNum);
                }
            }
        }
    }
