...
function pageInit(){
    ...
    const columns = columnsBuild()
    $('#tagtable').bootstrapTable({
        url:ME.host+'/super/tag/list',
        method:'get',
        ...
        responseHandler:function (list) {
            // console.log(list);
            if(list.some(tag => tag.otaProgress || tag.otaTime)){
                const options = $('#tagtable').bootstrapTable('getOptions');
                $('#tagtable').bootstrapTable('showColumn','otaProgress')
                $('#tagtable').bootstrapTable('showColumn','otaTime')
                options.sortName = 'otaProgress'
                $('#tagtable').bootstrapTable('refreshOptions',options);
            }
            var sameTagSlotSeqNum = [];
            var testobj={};
            for (var i=0;i<list.length;i++){
                var seqNum = list[i].tagSlotSeqNum;
                if(seqNum!==null) {
                    var key = seqNum + "";
                    if (!testobj[key]) {
                        testobj[key] = true;
                    } else {
                        sameTagSlotSeqNum.push(seqNum);
                    }
                }
            }
            ME.vm.$data.sameTagSlotSeqNum = sameTagSlotSeqNum;
            return list;
        },
    columns,
});
...
function vueInit(){
    ...
    var VUE_DATA = Object.assign(getCfgData(TAG_CONF),
        {
            ...
            // checkAllUpdate:[],
            // selectParams:{}, //选择的属性键值
            // isCheckAll:false,
            // selectedList:[],   //选择的属性
            // actionTypeBycheckBox:['update','updateMore'],


            checkAllUpdate:false,
            basicLabels: ['projectId','productId','rxDelay@txDelay', 'maxPower','manu'],
            basicSelected:[],//选择的属性

            setLabels:['locatingPeriod','txPower','wakePeriod','locationMode','switchDistance','sleepWait','isHoldPosition', 'tagSlotSeqNum',  'isHoldBle','isHoldUart','distanceDiffBias', 'alterMinDistance@alterMaxDistance', 'motionSensorMode'],
            setSelected:[],

            transmitLabels:['transmit'],
            transmitSelected:[],
    ...
    var VUE_METHODS = Object.assign(CONF_VUE_METHODS, {
        ...
        unlockTagDgClose:function () {
            this.unlockTagDg=false;
        },
        unlockTagDgOpen:function () {
            this.unlockTagDg=true;
            this.passErr = '';
        },
        checkPassWord:function () {
            var that =this;
            that.passErr = '';
            var str = "ubitraq"+moment().format('HHDDMM');
            if(that.passWord==str){
                that.lockTagSet = false;
                that.unlockTagDg=false;
                that.passWord = '';
            }else{
                that.passErr = '失败'
            }
        ...
        tagDelete:function(){
            ...
            this.$confirm(lang['tagDeleteNote'],lang['prompt'],{ }).then(function()  {
                ...
                ME.vm.$http.post('super/tag/delete', {ids:ids}).then(function(res){
                    var result = res.body;
                    if(result.isOk){
                        ME.vm.refresh();
        ...
        showMessageBox:function () {
            ...
            this.tagobjarray=rows;
        },
        correctTime:function () {
            var codes = this.getOnlineTagCodes();
            if(!codes) return;
            ME.vm.correctTimeBtnDisable = true;
            ME.vm.$http.post('super/tag/correctTime', {codes:codes,time:Date.now()}).then(function(res){
                ME.vm.response(res.body, codes, "校准时间", false);
                ME.vm.correctTimeBtnDisable = false;
            },function (err) {
                ME.vm.correctTimeBtnDisable = false;
            });
        },
        ...
        response:function (r, codes, title, isFresh) {
            if(r.hasOwnProperty('isOk')){
                if(r.msg) {
                    r.msg = r.msg.replace(/has no rangAnchor or posAnchor!/g, '无交互基站')
                }
            ...
            if(r.successful.length == 0){
                ...
                for(var f of r.failed){
                    if(f.msg) {
                        f.msg =f.msg.replace(/has no rangAnchor or posAnchor!/g, '无交互基站')
                    }
                    if(f.tag){
                        text += f.tag+f.msg+';';
                    }else{
        ...
        submitForm:function (formName) {
            ...
            this.$refs[formName].validate(function(valid){
                if (valid) {
                    ...
                    // 检查勾选，并获取值
                    if(this.checkAllUpdate || type=='add'){
                    ...
                    switch(type){
                        case 'add' :url+='add';
                            msg+=lang['addTag'];break;
                        case 'update': url+='update';
                            msg+=lang['updateTag'];break;
                        case 'updateMore':
                            url+='updateMore';
                            params.codes = params.codes.split(',');
                            msg+=lang['updateMoreTag'];break;
                        default:
        ...
        setTransmit:function(){
            var codes = this.getOnlineTagCodes();
            if(!codes) return false;
            if(this.transmitSelected.indexOf('transmit')==-1){
                ME.vm.$alert(lang['mustCheckParm'], lang['prompt'], {
                    confirmButtonText: lang['confirm']
                });
                return false;
            ...
            if(this.transmitSelected.indexOf('transmit')!=-1){
                ...
                params.pulseFrequency=this.form.pulseFrequency;
                params.preambleCode=this.form.preambleCode;
                params.preambleLength=this.form.preambleLength;
            }
            var that =this;
            this.$confirm("可能导致标签无法使用，确认"+msg+"标签么？","提示",{ }).then(function()  {
                that.setTransmitBtnDisable = true;
                that.$http.post('super/tag/setTag',params).then(function(res){
                    ME.vm.setTransmitBtnDisable = false;
                    ME.vm.response(res.body, codes, msg);
                })
            }).catch(function(){});
        },
        ...
        setRetryTm:function () {
            var that =this;
            ME.vm.$http.post('super/tag/setretrytimes', {times:that.setRetryTimes}).then(function(res){
                if(res.body.isOk){
                    that.retryTimes=res.body.times;
                    that.dialogTimesVisible=false;
                }else {
                    console.log(res);
                    this.$alert(res.body.msg, '错误提示', {
                        confirmButtonText: '确定',
                    });
                }
            });
        ...
        showRetryTm:function () {
            var that =this;
            ME.vm.$http.get('super/tag/getretrytimes').then(function(res){
                if(res.body.isOk){
                    that.retryTimes=res.body.times;
                    that.dialogTimesVisible=true;
                }else {
                    console.log(res);
                }
            });
        },
        dialogTimesClose:function () {
...
function columnsBuild(){
    return [
        ...
        {field: 'otaProgress', title: 'ota进度', width:'5%', sortable:true, searchable:true,visible:false,formatter:value => value?`${value}%`:'暂无'}, 
        {field: 'otaTime', title: 'ota时间', width:'15%', sortable:true, searchable:true,visible:false,formatter:value => value?moment(Number(value)).format(`YYYY-MM-DD HH:mm:ss`):'暂无' }, 
        ...
        {
            field: 'onlineTime', title: lang['onlineTime'], width: '6%',  sortable: true, searchable: true,
            formatter: function (value, row, index) {
                if (value) {
        ...
        {
            field: 'nbSqual', title: lang['nbSqual'], width: '6%',  sortable: true, searchable: true,
            formatter: function (value, row, index) {
                if (value) {
                    return value;
                }
        ...
        // {field: 'wakeFreq',title: lang['wakeFreq'],width:'25%', sortable:true, searchable:true},
        // {field: 'refreshFreq',title: lang['refreshFreq'],width:'10%', sortable:true, searchable:true},
        // {field: 'txPower',title: lang['txPower'],width:'10%', sortable:true, searchable:true},
        // {field: 'maxPower',title: lang['maxPower'],width:'10%', sortable:true, searchable:true},
        // {field: 'manu',title: lang['manu'],width:'10%', sortable:true, searchable:true},
        ...
        {field: 'tagSlotSeqNum',title: lang['tagSlotSeqNum'],width:'6%', sortable:true, searchable:true,
            formatter:function (value,row,index) {
                if(ME.vm.$data.sameTagSlotSeqNum.indexOf(value) !== -1){
