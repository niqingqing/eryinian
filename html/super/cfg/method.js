    ...
    cfSubmit:function (formName, url) {
        this.$refs[formName].validate(function(valid){
            if (valid) {
                ...
                //检查 测量距离配置是否正确
                for(var i=0;i<ME.vm.cfForm.cfg.length;i++){
                    if(ME.vm.cfForm.cfg[i].key === "cfg_anchor_distances"){
                        try{
                            JSON.parse(ME.vm.cfForm.cfg[i].value);
                        }catch(e){
                            console.log(e);
                            ME.vm.$alert("到其他基站的飞行距离:不是JSON", lang['prompt']);
                            return
                        }
                    }
                }
