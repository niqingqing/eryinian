...
function pageInit(){
    ...
    $('#forceRemovetable').bootstrapTable({
        ...
        queryParams:function(params){
            ...
            if(searchParams.mapId) params.mapId = searchParams.mapId;
    ...
    columns: [
        ...
        {field: 'power',title: '电量',width:'20%', sortable:true, searchable:true},
        {field: 'time',title: '时间',width:'20%', sortable:true, searchable:true,
            formatter:function(value, row, index){
            ...
            this.$http.get(ME.host+'/model/listBase?model=map').then(function(res){
                ...
                this.$http.get(ME.host+'/model/listBase?model=tag').then(function(res){
                    ...
                    if(this.tags.length>0){
                        this.searchform.tag=this.tags[0].code;
                    }
                    pageInit();
                });
            });
        ...
        methods:{
            ...
            clearTable:function(){
                ...
                this.$confirm(lang['clearTableNote'],lang['prompt'],{ }).then(function() {
                    ...
                    var tag = form.tagId;
                    var url = ME.host+'/super/power/clearTable?tag=' + tag + '&mapId=' + mapId + '&start=' + start + '&end=' + end;
            ...
            showChart:function () {
                this.powerChart=true;
                var sourceDatas = $('#forceRemovetable').bootstrapTable('getData');
                var arr = [];
                for(var i=0;i<sourceDatas.length;i++){
                    // var data = [new Date(sourceDatas[i].time).toLocaleString(),sourceDatas[i].power];
                    var data = [new Date(sourceDatas[i].time).valueOf(),sourceDatas[i].power];
                    arr.push(data);
                }
                this.drawChart(arr);
            },
            closeChart:function () {
                this.powerChart=false;
            },
            drawChart(data){
