...
function pageInit(){
    ...
    $('#tagtable').bootstrapTable({
        ...
        responseHandler:function(list){
            ...
            var anchors = ME.vm.asyncGetAnchors();
            ME.vm.checkSlotSeqNum(list,anchors);
            return list;
        },
        ...
        {field: 'tagSlotSeqNum',title: lang['tagSlotSeqNum'],width:'6%', sortable:true, searchable:true,
            formatter: function (value, row, index) {
                if(value||value===0||value==='0') {
                    if (ME.vm.repeatSlotSeqNums.indexOf(value) != -1) {
        ...
        sendMessage:function () {
            ...
            this.$confirm("确认发送？","提示",{ }).then(function()  {
                ME.vm.$http.post('super/tag/sendMessage', {codes:codes,message:message}).then(function(res){
                    ME.vm.response(res.body, codes, "发送消息");
                    ME.vm.isSending = false;
                });
                ME.vm.isSending = true;
            }).catch(function(){
            });
        },
        checkSlotSeqNum:function (tags,anchors) {
            var seqNum={};
            for (var i = 0; i < anchors.length; i++) {
                var a = anchors[i];
                if (a.isMaster > 0) {
                    if (a.anchorSlotSeqNum || a.anchorSlotSeqNum === 0 || a.anchorSlotSeqNum === '0') {
                        if (!seqNum[a.anchorSlotSeqNum]) {
                            seqNum[a.anchorSlotSeqNum] = true;
                        }
                    }
                }
            }
            for(var j=0;j<tags.length;j++){
                var t=tags[j];
                if(t.tagSlotSeqNum || t.tagSlotSeqNum===0 || t.tagSlotSeqNum==='0'){
                    if(!seqNum[t.tagSlotSeqNum]){
                        seqNum[t.tagSlotSeqNum]=true;
                    }else {
                        ME.vm.repeatSlotSeqNums.push(t.tagSlotSeqNum);
                    }
                }
            }
        },
