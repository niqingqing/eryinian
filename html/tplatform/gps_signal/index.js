...
function vueInit(){
    ...
    ME.vm = new Vue({
        el:'#app',
        data:{
            type:Cookies.get('source_type')?Cookies.get('source_type'):'proocess_squal',
            // type:'proocess_squal',
            debugBuf:false,
            //按钮控制
            ...
            sourceTypes:[
                {value:'info',label:'gps定位'},
                {value:'preprocess_squal',label:'NBIOT信号质量和GNSS的状态'},
                {value:'preprocess_nb',label:'NBIOT信号质量'},
                {value:'preprocess_gnss',label:'GNSS各个卫星的信号质量'},
                {value:'preprocess_err',label:'错误'},
        ...
        created:function(){
            ...
            if(!Cookies.get('source_type')){
                Cookies.set('source_type', 'proocess_squal');
            }
            this.type = Cookies.get('source_type');


            if(!(UBIT.user.projectCode &&  UBIT.user.projectId>0)){
                alert('必须进入项目才能查看Gps信号情况！')
                return false;
            }

            //获取项目下所有地图
            this.currentProj = UBIT.user.projectCode;
            this.currentMap= this.currentProj+"_gps"
            if(!this.currentMap){return}
            this.maps=[{cname:UBIT.user.projectName,id:'gps'}];
