...
function dataInit3d(datas, key, vm){
    // console.dir(key);
    if(_.isArray(datas)){
        vm[key].data = datas;
    }else {
    ...
    if(key=='mapId'){
        ...
        if(datas>0){
            ...
            ME.messages.showTip("您当前的项目还没有创建地图", UBIT.selfHost + "/super/index/");
            return;
        }
        ...
        nodeInit(ME, vm, key);
        webSocketInit('3D', vm.curRealLength, {sos:sosAlert, coord:websocketOnData, fenceAlert:fenceAlert});
        render();
    }
}
...
function addShape( shape, fence) {
    ...
    var extrudeSettings = { amount: height, bevelEnabled: true, bevelSegments: 20, steps: 2, bevelSize: 10, bevelThickness: 10 };
    // extruded shape
    ...
    for(var i=0;i<points.vertices.length;i++){
        ...
        geometry.vertices.push(new THREE.Vector3(v.x, v.y, height));
...
function planInit(ME, vm){
    ...
    var imgUrl = UBIT.getImgSrc('maps', vm.currentMap.filePath);
    ...
    loader.load(
        ...
        function ( texture ) {
            ...
            ME.mapHeight = texture.image.height;
            ME.mapWidth = texture.image.width;
            ...
            //加载地图上的标签
            DataManager.loadNodes(vm.mapId, vm, dataInit3d, convertFunc);

            //加载地图上的fence
            DataManager.loadFences(vm.mapId, vm, dataInit3d, convertFunc);
        },
...
function  sosAlert(data){
    console.warn('sosAlert:',data);
}

function  fenceAlert(data){
    console.warn('fenceAlert:',data);
}

function  websocketOnData(data, realLength){
    ...
    //过滤 currentTag
    if(ME.vm.currentTag ){
        if(!data || data.code != ME.vm.currentTag.code){
            return ;
        }
    }

    var tag = getTagMarker(data.code);
    if(!tag){
