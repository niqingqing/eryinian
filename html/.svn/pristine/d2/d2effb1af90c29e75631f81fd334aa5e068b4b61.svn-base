    ...
    ME.vm = new Vue({
        ...
        methods: {
            set:function(){
                ...
                var stillnessType = this.stillnessType;
                var distance = this.distance;
                if(!this.stillnessType){
                    return this.$alert('请选择聚集类型','提示');
                }
                if(!this.map){
                    return this.$alert('请选择地图','提示');
                }
                var url = ME.host + '/super/stillness/addSetting?id=' + this.map + '&duration=' + this.duration + '&period=' + periods + '&stillnessType=' + stillnessType + '&distance=' + distance;
            ...
            setAll:function(){
                ...
                var url = ME.host + '/super/stillness/setAll?duration=' + this.duration + '&period=' + periods + '&stillnessType=' + stillnessType  + '&distance=' + distance;
                this.$http.get(url).then(function(res){
                    this.periods = [];
                    var type = 'success';
                    if(!res.body.isOk){
            ...
            remove:function(){
                ...
                var url = ME.host + '/super/stillness/deleteSetting?id=' + id;
                this.$http.get(url).then(function(res){
                    var type = 'success';
                    if(!res.body.isOk){
                        type = 'error';
            ...
            removeAll:function(){
                var self = this;
                var url = ME.host + '/super/stillness/deleteAllSetting'
                this.$http.get(url).then(function(res){
                    var type = 'success';
                    if(!res.body.isOk){
                        type = 'error';
            ...
            setTypeCommit:function(){
                ...
                var obj = {
                    ...
                    type:this.stillnessType
                }
                var url = ME.host + '/super/stillness/setStillnessType';
            ...
            getTypeList:function(){
                var self = this
                this.$http.get(ME.host + '/super/stillness/getAllStillnessTypes').then((res) => {
...
function initTable() {
    ...
    $('#table').bootstrapTable({
        url: ME.host + '/super/stillness/settingList',
        method: 'get',
        ...
        columns: [
            ...
            { field: 'stillnessType', title: "聚集类型", width: '19%', sortable: true, searchable: true },
...
function initTypeTable(){
    ...
    $('#typeTable').bootstrapTable({
        url: ME.host + '/super/stillness/tagStillnessTypeList',
        method: 'get',
        queryParams: function (params) {
            return params;
        },
        ...
        columns: [
            ...
            { field: 'aggregateType', title: '标签类型', width: '33%', sortable: true, searchable: true },
        ]
    });
}